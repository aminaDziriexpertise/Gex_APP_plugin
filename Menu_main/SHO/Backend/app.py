...
#url = 'https://odoo.gexpertise.fr/web#id=94933&action=311&active_id=18&model=helpdesk.ticket&view_type=form&menu_id>   new_url = 'https://odoo.gexpertise.fr/web#id=94933&action=311&active_id=18&model=helpdesk.ticket&view_type=form&m>   url = requests.get('http://10.252.216.10:5080/')
...
#**********************Cree le **********************
   ...
   Cree_le_extension = requests.post('http://10.252.216.10:5080/cree_le_extension', data=Cree_le_extension)   
   ...
   print('RDE :', RDE)
   RDE = RDE.split("RDE :")[1]
   rde = requests.post('http://10.252.216.10:5080/RDE', data=(fxn(RDE)))
 ...
 #**********************Full_address**********************
   ...
   if commune.startswith("PARIS"):
      ...
      commune = requests.post('http://10.252.216.10:5080/commune', data=commune)
   else:
      commune =  commune.upper()
      commune = requests.post('http://10.252.216.10:5080/commune', data=commune)
 ...
 #**********************long__**********************
   ...
   adress = requests.post('http://10.252.216.10:5080/address1', data=address1)


   Mission = requests.post('http://10.252.216.10:5080/Mission', data=Mission)
   num_livrable = requests.post('http://10.252.216.10:5080/num_livrable', data=num_livrable)
   Cree_le = requests.post('http://10.252.216.10:5080/Cree_le', data=Cree_le)
   Cree_par = requests.post('http://10.252.216.10:5080/Cree_par', data=Cree_par)
   destination = requests.get('http://10.252.216.10:5080/destination')
   driver.quit()
   ...
   def localisation_extract_v01(adresse):
              ...
              if "housenumber" in properties:
                   ...
                   number = requests.post('http://10.252.216.10:5080/number', data=number)
              else:
                   number = ""
                   number = requests.post('http://10.252.216.10:5080/number', data=number)
      # extraction du département à partir de la réponse JSON
              ...
              if city == "Paris":
                     ...
                     if (a in distinct):
                          ...
                          distinct = "("+distinct+")"
                          departement = requests.post('http://10.252.216.10:5080/departement', data=distinct)
                     else:
                          print("distinct", distinct)
              else:
                    ...
                    code_postale = requests.post('http://10.252.216.10:5080/code_postale', data=code_postal)
                    departement_name = "("+code_postal + "-" + nom_departement+")"
                    departement = requests.post('http://10.252.216.10:5080/departement', data=departement_name)
             ...
             # number = response.json()["features"][0]["properties"]["housenumber"]
              ...
              destination = requests.get('http://10.252.216.10:5080/destination')
              #number = requests.post('http://10.252.216.10:5080/number', data=number)
              street = requests.post('http://10.252.216.10:5080/street', data=street)
   ...
   def localisation_extract(address):
            ...
            city = requests.post('http://10.252.216.10:5080/city', data=city)
   ...
   def cadastre_extract():
          ...
          if features and len(features) > 0:
               ...
               cadastre = requests.post('http://10.252.216.10:5080/cadastre', data=cadastre)
         ...
         # if commune_name == "Paris":
          ...
          #   commune = requests.post('http://10.252.216.10:5080/commune', data=commune_name)
          #else:
          ...
          cadastre = requests.post('http://10.252.216.10:5080/cadastre', data=cadastre)
          section_num = requests.post('http://10.252.216.10:5080/section_num', data=section_num)
          section_name = requests.post('http://10.252.216.10:5080/section_name', data=section_name)
